...
class ProviderSelector:
    ...
    def select_provider(self, req_vision: bool, est_tokens: int) -> Optional[ProviderDef]:
        scored = []
        ...
        if not scored:
            ...
            setattr(fallback, 'requires_pause_after_unit', True)
            return fallback
            
        scored.sort(key=lambda x: x[0], reverse=True)
        chosen = scored[0][1]
        setattr(chosen, 'requires_pause_after_unit', False)
        
        # Si se eligió local porque no quedaban nubes sanas, marcamos la pausa
        if chosen.kind == "local":
            setattr(chosen, 'requires_pause_after_unit', True)
