...
async def probar_juego_interactivo(comando: list[str],
                                   ...
                                   marca_fichero: Path | None = None,
                                   ) -> tuple[str, str]:
    """
    Lanza `comando` (p. ej. `[exe, "--autotest"]`) y devuelve
    `(estado, salida)` con `estado` en:

      · "verde"          — código 0 Y la marca: impresa en la salida O
                           escrita en `marca_fichero` (los exe de ventana
                           NO tienen consola: el print muere en un stdout
                           nulo — hallado con el propio fixture de Tetris).
      ...
      · "sin_comprobar"  — cuelgue o lanzamiento imposible. La regla de la
                           casa: no comprobado NO es roto.

    Ni el código solo ni la marca sola bastan: un juego que sale 0 sin
    probar las teclas no probó nada.
    """
    if marca_fichero is not None and marca_fichero.exists():
        marca_fichero.unlink()          # la marca de una corrida anterior
    try:
    ...
    if proc.returncode == 0 and (marcador in texto or (
            marca_fichero is not None and marca_fichero.exists())):
        return "verde", texto[-2000:] or f"(sin salida; marca en {marca_fichero})"
